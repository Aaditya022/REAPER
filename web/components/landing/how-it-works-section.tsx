...
const steps = [
  {
    ...
    description: "One command starts the flow. Pick your frontend, backend, and database through interactive prompts.",
    code: `reaper create

? Select a frontend framework › React
? Select a backend framework › ExpressJS
? Select a database › PostgreSQL
? Deploy this to Zerops now? › yes`,
  },
  {
    number: "02",
    title: "Generate",
    subtitle: "the config",
    description: "REAPER detects the project architecture and validates your environment before generating the deployment configuration.",
    code: `Detected stack:
  framework     express
  port          8080

✓ environment validated
✓ zerops.yaml generated
...
Deploying to Zerops...`,
  },
  {
    ...
    description: "Deployment status is tracked state by state, and your live URL is health-checked over HTTPS before it ships.",
    code: `state=PENDING
state=ANALYZING
state=CONFIGURING
state=DEPLOYING
...
export function HowItWorksSection() {
  ...
  return (
    <section
      id="how-it-works"
      ref={sectionRef}
      className="relative pt-16 lg:pt-24 pb-24 lg:pb-28 bg-[oklch(0.09_0.01_260)] text-white overflow-hidden"
    >
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto px-6 lg:px-12">
        ...
        <div className="relative mb-0 lg:mb-0 grid lg:grid-cols-2 gap-4 lg:gap-12 items-end">
          ...
          <div className="overflow-hidden pb-0 lg:pb-[25rem]">
            ...
            <h2 className={`text-[2rem] md:text-[2.5rem] lg:text-[3.25rem] font-display font-bold tracking-[-0.01em] leading-[1.15] lg:origin-top-left lg:scale-[3] transition-all duration-1000 delay-100 ${
              ...
              <span className="block accent-word">Deploy.</span>
              <span className="block text-white/45">Verify.</span>
            </h2>
          </div>
          ...
          {steps.map((step, index) => (
            ...
            >
              ...
              <div className="flex items-center gap-4 mb-8">
                <span className={`text-4xl font-display font-bold transition-colors duration-300 ${
              ...
              {/* Title */}
              <h3 className="text-[1.5rem] lg:text-[1.875rem] font-display font-semibold leading-[1.25] mb-2">
                {step.title}
              </h3>
              <span className="text-xl text-white/45 font-display font-medium block mb-6">
                {step.subtitle}
              </span>

              {/* Description */}
              <p className={`text-lg text-white/65 leading-[1.6] transition-opacity duration-300 ${
        ...
        <div className="mt-16 border border-white/20 bg-black overflow-hidden">
          <div className="flex items-center gap-2 px-6 py-4 border-b border-white/10">
            <span className="w-3 h-3 rounded-full bg-white/20" />
            <span className="w-3 h-3 rounded-full bg-white/20" />
            <span className="w-3 h-3 rounded-full bg-white/20" />
            <span className="ml-4 font-mono text-sm text-white/50">
              reaper — {steps[activeStep].title.toLowerCase()}
            </span>
          </div>
          <pre className="p-8 font-mono text-sm leading-relaxed text-white/80 whitespace-pre-wrap">
